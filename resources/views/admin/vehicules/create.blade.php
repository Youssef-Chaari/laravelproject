...
<div class="page-header">
    <div>
        ...
        <p>Création d'une nouvelle fiche véhicule (catalogue du neuf).</p>
    </div>
    ...
    <form action="{{ route('admin.vehicules.store') }}" method="POST">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-bottom:24px">
            ...
            <div class="form-group">
                ...
                <input type="text" id="modele" name="modele" class="form-input" required value="{{ old('modele') }}">
            </div>

            <div class="form-group">
                <label class="form-label" for="annee">Année</label>
                <input type="number" id="annee" name="annee" class="form-input" required min="1900" max="{{ date('Y') + 1 }}" value="{{ old('annee', date('Y')) }}">
            </div>

            <div class="form-group">
                <label class="form-label" for="prix">Prix (€)</label>
                <input type="number" id="prix" name="prix" class="form-input" required min="0" step="1" value="{{ old('prix') }}">
            </div>
            ...
            <div class="form-group">
                ...
                <input type="number" id="puissance" name="puissance" class="form-input" value="{{ old('puissance') }}" min="0" required>
            </div>

            <div class="form-group">
                <label class="form-label" for="couple">Couple (Nm)</label>
                <input type="number" id="couple" name="couple" class="form-input" value="{{ old('couple') }}" min="0">
            </div>

            <div class="form-group">
                <label class="form-label" for="consommation">Consommation (L/100km)</label>
                <input type="number" id="consommation" name="consommation" class="form-input" value="{{ old('consommation') }}" min="0" step="0.1">
            </div>

            <div class="form-group">
                <label class="form-label" for="nb_portes">Nb Portes</label>
                <input type="number" id="nb_portes" name="nb_portes" class="form-input" value="{{ old('nb_portes', 5) }}" min="2" max="5">
            </div>

            <div class="form-group">
                <label class="form-label" for="nb_places">Nb Places</label>
                <input type="number" id="nb_places" name="nb_places" class="form-input" value="{{ old('nb_places', 5) }}" min="1" max="9">
            </div>

            <div class="form-group">
                <label class="form-label" for="volume_coffre">Volume Coffre (L)</label>
                <input type="number" id="volume_coffre" name="volume_coffre" class="form-input" value="{{ old('volume_coffre') }}" min="0">
            </div>

            <div class="form-group">
                <label class="form-label" for="couleur">Couleur (nom)</label>
                <input type="text" id="couleur" name="couleur" class="form-input" value="{{ old('couleur') }}" placeholder="ex: Bleu Nuit">
            </div>

            <div class="form-group">
                <label class="form-label" for="couleur_bg">Couleur (Hex)</label>
                <input type="color" id="couleur_bg" name="couleur_bg" class="form-input" value="{{ old('couleur_bg', '#3b82f6') }}" style="height:44px;padding:4px">
            </div>

            <div class="form-group">
                <label class="form-label" for="kilometrage">Kilométrage (si expo)</label>
                <input type="number" id="kilometrage" name="kilometrage" class="form-input" value="{{ old('kilometrage', 0) }}" min="0">
            </div>

            <div class="form-group">
                <label class="form-label" for="garantie">Garantie (mois)</label>
                <input type="number" id="garantie" name="garantie" class="form-input" value="{{ old('garantie', 24) }}" min="0">
            </div>
        </div>

        <div class="form-group">
            <label class="form-label" for="equipements">Équipements (séparés par des virgules)</label>
            <input type="text" id="equipements" name="equipements" class="form-input" value="{{ old('equipements') }}" placeholder="ex: GPS, Toit ouvrant, Sièges chauffants">
        </div>
